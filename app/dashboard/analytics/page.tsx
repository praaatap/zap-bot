    ...
    useEffect(() => {
        async function fetchStats() {
            try {
                ...
                // Set empty stats when API fails - no hardcoded data
                setStats({
                    totalMeetings: 0,
                    hoursTranscribed: 0,
                    recordingsCount: 0,
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600 leading-relaxed font-medium">
                                {stats?.totalMeetings > 0 
                                    ? `ZapBot has processed ${stats.totalMeetings} meetings totaling ${stats.hoursTranscribed} hours of transcription. ${stats.recordingsCount} recordings are stored.`
                                    : "No meetings processed yet. Connect your calendar and dispatch a bot to get started."
                                }
                            </p>
                        </div>
                        ...
                        <div className="space-y-4 pt-4 border-t border-slate-200">
                            <div>
                                <div className="flex justify-between text-[11px] font-bold text-slate-500 mb-2">
                                    ...
                                    <span>{stats?.totalMeetings > 0 ? Math.round((stats.recordingsCount / stats.totalMeetings) * 100) : 0}%</span>
                                </div>
                                <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                                    <motion.div
                                        initial={{ width: 0 }}
                                        animate={{ width: `${stats?.totalMeetings > 0 ? Math.min(100, Math.round((stats.recordingsCount / stats.totalMeetings) * 100)) : 0}%` }}
                            ...
                            <div>
                                <div className="flex justify-between text-[11px] font-bold text-slate-500 mb-2">
                                    ...
                                    <span>{stats?.totalMeetings > 0 ? Math.round((stats.hoursTranscribed / Math.max(1, stats.totalMeetings)) * 10) : 0}%</span>
                                </div>
                                <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                                    <motion.div
                                        initial={{ width: 0 }}
                                        animate={{ width: `${stats?.totalMeetings > 0 ? Math.min(100, Math.round((stats.hoursTranscribed / Math.max(1, stats.totalMeetings)) * 10)) : 0}%` }}
