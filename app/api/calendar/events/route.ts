        ...
        const meetingEvents = await Promise.all(
            processedEvents.map(async (event: any) => {
                ...
                try {
                    ...
                    // Check if a meeting with this calendarEventId already exists
                    const existingResult = await databases.listDocuments(
                        APPWRITE_IDS.databaseId,
                        APPWRITE_IDS.meetingsCollectionId,
                        [
                            Query.equal("calendarEventId", event.id),
                            Query.limit(1),
                        ],
                    );
                    ...
                    if (existingResult.documents.length > 0) {
                        ...
                        dbMeeting = await databases.updateDocument(
                            APPWRITE_IDS.databaseId,
                            APPWRITE_IDS.meetingsCollectionId,
                        ...
                        // Create new meeting
                        dbMeeting = await databases.createDocument(
                            APPWRITE_IDS.databaseId,
                            APPWRITE_IDS.meetingsCollectionId,
                            ID.unique(),
                            {
